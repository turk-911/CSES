...
double calculateProbability(int n, int a, int b) {
    vvi dp(n + 1, vi(6 * n + 1, 0));
    ...
    int outcomes = 1;
    for(int i = 0; i < n; i++) outcomes *= 6;
    int fav = 0;
    for(int i = a; i <= b; i++) fav += dp[n][i];
    return static_cast<double>(fav) / outcomes;
}
void solve() {
    int n, a, b;
    cin >> n >> a >> b;
    double prob = calculateProbability(n, a, b);
